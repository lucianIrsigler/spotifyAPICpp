...
/*
user-follow-modify*/
void spotifyFollowers::unfollowArtist(std::string type, std::string IDS, std::vector<std::string>artistIDS) {
	...
	if (IDS != "") {
		...
		std::string readBuffer = utility::performCURLDELETE(url, "", spotifyAuthenticityToken);
		...
		std::string readBuffer = utility::performCURLDELETE(url, JSONobject, spotifyAuthenticityToken);
...
void spotifyFollowers::followPlaylist(std::string playlistID, bool isPublicOnProfile) {
	...
	std::string readBuffer = utility::performCURLPUT(url, JSONbody, spotifyAuthenticityToken);
